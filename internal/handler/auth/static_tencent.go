...
func StaticTencentHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.TencentAuthReq
		if err := httpx.Parse(r, &req); err != nil {
			response.ErrorCtx(r.Context(), w, errs.WithCode(err, errs.ErrCodeParamsAbnormal), nil)
			return
		}

		l := auth.NewStaticTencent(r.Context(), svcCtx)
		resp, err := l.StaticTencent(&req)
		if err != nil {
