...
var (
	...
	smsRecordRowsExpectAutoSet   = strings.Join(stringx.Remove(smsRecordFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`delete_at`", "`delete_time`", "`deleted_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	smsRecordRowsWithPlaceHolder = strings.Join(stringx.Remove(smsRecordFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`delete_at`", "`delete_time`", "`deleted_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"
)
...
func (m *defaultSmsRecordModel) Update(ctx context.Context, data *SmsRecord) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.Phone, data.Code, data.Message, data.Channel, data.Template, data.Status, data.Id)
	return err
}
